...
/*   Updated: 2015/08/22 18:42:20 by ngoguey          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				sp_init_sbox(t_env *e)
{
	...
	glGenTextures(1, &e->sbox_map);
	glBindTexture(GL_TEXTURE_CUBE_MAP, e->sbox_map);
	i = 0;
	while (i < 6)
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i++
			, 0, GL_DEPTH_COMPONENT
			, e->sbox_resolution, e->sbox_resolution, 0
	...
	glFramebufferTexture(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, e->sbox_map, 0);
